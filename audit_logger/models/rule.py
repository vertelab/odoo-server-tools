...
class AuditlogRule(models.Model):
    ...
    field_ids = fields.Many2many(
        ...
        string="Fields",
        help="Select fields for which you want to generate log. If left empty all will be logged",
        states={"subscribed": [("readonly", True)]},
        domain="[('model_id','=',model_id), ('name','not in',%s)]" % FIELDS_BLACKLIST,
    )
    ...
    def _make_read(self):
        ...
        self.ensure_one()
        log_type = self.log_type
        field_filter = self.field_ids.mapped("name")
        ...
        def read(self, fields=None, load="_classic_read", **kwargs):
            ...
            rule_model.sudo().create_logs(
                ...
                read_values,
                None,
                {"log_type": log_type, "field_filter": field_filter}
                if field_filter
                else {"log_type": log_type},
            )
            return result
    ...
    def create_logs(
        ...
        field_filter = ""
        if additional_log_values.get("field_filter"):
            field_filter = additional_log_values.pop("field_filter")
        for res_id in res_ids:
            ...
            if method == "create":
                self._create_log_line_on_create(
                    log, diff.added(), new_values, field_filter
                )
            elif method == "read":
                ...
                self._create_log_line_on_write(
                    log, diff.changed(), old_values, new_values, field_filter
                )
    ...
    def _create_log_line_on_read(self, log, fields_list, read_values, field_filter):
    ...
    def _create_log_line_on_create(self, log, fields_list, new_values, field_filter):
